...
async fn test_agent_direct_llm_response() {
    ...
    let user_message =
        jarvis_rust::history::Message::user(session_id.to_string(), user_input.to_string());
    history.save(user_message).await.unwrap();

    // Test saving assistant response to history
    let assistant_message = jarvis_rust::history::Message::assistant(
...
async fn test_agent_fsm_with_tool_calls() {
    ...
    assert_eq!(
        *fsm.current_state(),
        jarvis_rust::agent::fsm::AgentState::ReadyToCallLlm
    );
    ...
    assert_eq!(
        *fsm.current_state(),
        jarvis_rust::agent::fsm::AgentState::AwaitingLlmResponse
    );
    ...
    assert_eq!(
        *fsm.current_state(),
        jarvis_rust::agent::fsm::AgentState::ExecutingTools
    );
    ...
    assert_eq!(
        *fsm.current_state(),
        jarvis_rust::agent::fsm::AgentState::Done
    );
...
async fn test_agent_fsm_error_handling() {
    use jarvis_rust::agent::fsm::{AgentEvent, AgentState, AgentStateMachine};
...
async fn test_conversation_history_flow() {
    ...
    let conversation_flow = vec![
        ...
        (
            "assistant",
            "Paris has approximately 2.1 million inhabitants in the city proper.",
        ),
        ("user", "Thanks!"),
        (
            "assistant",
...
async fn test_concurrent_sessions() {
    ...
    let history = std::sync::Arc::new(
        HistoryStorage::new(&db_path.to_string_lossy())
            .await
            .unwrap(),
    );
...
async fn test_tool_to_client_mapping_logic() {
    ...
    assert_eq!(
        tool_to_client_map.get("get_weather"),
        Some(&"weather_client".to_string())
    );
    assert_eq!(
        tool_to_client_map.get("send_email"),
        Some(&"weather_client".to_string())
    );
    assert_eq!(
        tool_to_client_map.get("search_web"),
        Some(&"search_client".to_string())
    );
    assert_eq!(
        tool_to_client_map.get("calculate"),
        Some(&"search_client".to_string())
    );
