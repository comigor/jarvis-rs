...
1. Re-read .openhands/microagents/CLAUDE.md RIGHT NOW
...
- Run `.openhands/pre-commit.sh` directly to see current state
- If ANY issues exist, they MUST be fixed before proceeding
- Check `.openhands/pre-commit.sh` if it exists
...
5. **NO STOPPING** - Keep working until ALL checks pass
