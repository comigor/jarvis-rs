...
impl HttpMcpClient {
    pub async fn new(config: McpServerConfig) -> Result<Self> {
        let base_url = config.url.ok_or_else(|| {
            Error::mcp("HTTP MCP client requires URL field")
        })?;

        debug!("Creating HTTP MCP client for: {}", config.name);
    ...
    async fn send_request(&self, method: &str, params: Value) -> Result<Value> {
        ...
        let response = req_builder.send().await.map_err(|e| {
            Error::mcp(format!("Failed to send HTTP MCP request: {}", e))
        })?;

        let response_json: Value = response.json().await.map_err(|e| {
            Error::mcp(format!("Failed to parse HTTP MCP response: {}", e))
        })?;

        // Check for JSON-RPC error
        if let Some(error) = response_json.get("error") {
            return Err(Error::mcp(format!("HTTP MCP error: {}", error)));
        }

        response_json.get("result").cloned().ok_or_else(|| {
            Error::mcp("HTTP MCP response missing result field")
        })
    }
}
...
impl McpClient for HttpMcpClient {
    async fn initialize(&mut self, request: McpInitializeRequest) -> Result<McpInitializeResponse> {
        debug!("Initializing HTTP MCP client: {}", self.name);
        ...
        debug!("Successfully initialized HTTP MCP client: {}", self.name);
        Ok(response)
    }
    ...
    async fn list_tools(&self) -> Result<Vec<McpTool>> {
        ...
        debug!("Listing tools for HTTP MCP client: {}", self.name);
        ...
        debug!("Found {} tools for HTTP MCP client: {}", mcp_tools.len(), self.name);
        Ok(mcp_tools)
    }
    ...
    async fn call_tool(&self, request: McpToolCallRequest) -> Result<McpToolCallResponse> {
        ...
        debug!("Calling tool '{}' for HTTP MCP client: {}", request.name, self.name);
    ...
    async fn list_prompts(&self) -> Result<Vec<McpPrompt>> {
        ...
        debug!("Listing prompts for HTTP MCP client: {}", self.name);
        ...
        debug!("Found {} prompts for HTTP MCP client: {}", mcp_prompts.len(), self.name);
        Ok(mcp_prompts)
    }
    ...
    async fn get_prompt(&self, request: McpGetPromptRequest) -> Result<McpGetPromptResponse> {
        ...
        debug!("Getting prompt '{}' for HTTP MCP client: {}", request.name, self.name);
    ...
    async fn close(&mut self) -> Result<()> {
        debug!("Closing HTTP MCP client: {}", self.name);
