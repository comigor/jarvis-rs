...
impl HttpMcpClient {
    pub async fn new(config: McpServerConfig) -> Result<Self> {
        let base_url = config
            .url
            .ok_or_else(|| Error::mcp("HTTP MCP client requires URL field"))?;
    ...
    async fn send_request(&self, method: &str, params: Value) -> Result<Value> {
        ...
        let response = req_builder
            .send()
            .await
            .map_err(|e| Error::mcp(format!("Failed to send HTTP MCP request: {}", e)))?;

        let response_json: Value = response
            .json()
            .await
            .map_err(|e| Error::mcp(format!("Failed to parse HTTP MCP response: {}", e)))?;
        ...
        response_json
            .get("result")
            .cloned()
            .ok_or_else(|| Error::mcp("HTTP MCP response missing result field"))
    }
}
...
impl McpClient for HttpMcpClient {
    ...
    async fn list_tools(&self) -> Result<Vec<McpTool>> {
        ...
        debug!(
            "Found {} tools for HTTP MCP client: {}",
            mcp_tools.len(),
            self.name
        );
        Ok(mcp_tools)
    ...
    async fn call_tool(&self, request: McpToolCallRequest) -> Result<McpToolCallResponse> {
        ...
        debug!(
            "Calling tool '{}' for HTTP MCP client: {}",
    ...
    async fn list_prompts(&self) -> Result<Vec<McpPrompt>> {
        ...
        debug!(
            "Found {} prompts for HTTP MCP client: {}",
            mcp_prompts.len(),
            self.name
        );
        Ok(mcp_prompts)
    ...
    async fn get_prompt(&self, request: McpGetPromptRequest) -> Result<McpGetPromptResponse> {
        ...
        debug!(
            "Getting prompt '{}' for HTTP MCP client: {}",
            request.name, self.name
        );
